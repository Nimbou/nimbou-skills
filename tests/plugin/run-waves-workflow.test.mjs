...
test('run-waves smokes the browser when a wave touched frontend files', async () => {
  ...
  assert.match(smoke.prompt, /Playwright[^\n]*fallback/i, 'the workflow must preserve the final automatic fallback')
  assert.doesNotMatch(smoke.prompt, /Playwright only when the user explicitly requested/i)
