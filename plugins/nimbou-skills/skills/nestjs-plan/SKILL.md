...
13. **Write-set completeness:** every concrete file path named anywhere in a task's body — including a one-line edit that reads as obvious, like declaring the inverse side of a relation — appears in that task's `Files`. An implementer is instructed to stop rather than write outside its declared boundary, so a path the task mentions but does not declare blocks the wave. "It is one line" is precisely the case that gets left out.
    For each new or changed Prisma relation, inspect both model definitions in the target repository, including split schema files. Add the file containing the inverse relation to the same schema task's `Files`, even if the task prose only names the owning model. Run the planned Prisma validation against the complete schema before handoff when the environment permits.
...
15. **Domain-to-storage reconciliation:** for each promised snapshot, revision history, or audit record, identify the exact persisted fields and the write/read path that preserves the value as of that revision. Current mutable columns are insufficient when the domain requires historical state. Include schema, migration/backfill, repository, and scoped tests in the plan where needed.
16. **Backfill-to-contract reconciliation:** inspect every generated value in planned migrations and backfills against the domain and OpenAPI representation, including `type`, `format`, nullability, uniqueness, and cardinality. A deterministic 32-character hex string is not a canonical UUID merely because it has 128 bits; if OpenAPI says `format: uuid`, specify a UUID representation and verification before handoff.
17. **Behavior-to-entrypoint coverage:** turn each action in the domain scenarios and task checklists into a concrete flow inventory: create, update, submit, approve, synchronize, and any other relevant verb. Locate the existing use-case/controller/job for every applicable flow and its scoped test. If a rule must run on both update and submit, prove whether one shared service already covers both; otherwise include both entrypoints, tests, and files in the plan. A checklist promise without an owner in `Files` is incomplete.
