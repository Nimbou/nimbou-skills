...
**REQUIRED:** an approved brief from `nimbou-cms-think`, and `docs/nimbou-cms-guidelines.md` for the mechanics. Don't re-derive the design here — plan it.

## When to use

- Right after `nimbou-cms-think` produces an approved design for a content feature.
...
**1 — Build the module through the authenticated dev-admin REST (real DDL) + enter content when content is in scope.** Create it with `POST /rest/modules` using the view type + fields from the brief, pinned to the catalog. Building through the admin entry point runs the real `afterAddModule`/`Field::add`/`ImageFile::afterAdd` — so columns, seed row (Item), and FK names are correct by construction. Upload images and set `active=1` only when the approved plan includes content entry.
