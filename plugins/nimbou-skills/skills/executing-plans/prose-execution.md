...
1. Dispatch **one** `spawn_agent` worker with `nimbou-skills:browser-smoke` in **`report` mode**, giving it `WORKTREE_ROOT`, the plan path, and the frontend files the run committed. Instruct it to read the `browser-smoke` skill before acting. That skill owns the mechanics: driver selection (Codex integrated browser first when available, then Chrome DevTools MCP, and Playwright only when explicitly requested), bringing the app up, the health check, deriving flows from the plan's promises, and evidence. `report` mode matters — the fix cycle and every commit stay here.
2. **`SKIPPED` is not a failure.** No allowed browser driver after checking that ordered selection, or an app that never came up, says nothing about the code. A Chrome DevTools headful failure is not a missing driver while Codex's integrated browser is available. Record the reason as a `concern` and stop the step. Never open a fix cycle on it, and never report the change as verified.
